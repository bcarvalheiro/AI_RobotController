...
public class Simulator {
	...
	private void loadMushrooms() {
		try {
			...
			for (int i = 0; i < 8; i++)
				s.nextLine();
			while (s.hasNext()) {
				getMushrooms().add(new Mushroom(s));
				System.out.println("Added mushroom " + s);
			}
		} catch (FileNotFoundException e) {
			System.err.println("Error loading mushroom linkedlist");
			e.printStackTrace();
		}
	}
	...
	private void addMushroom() {
		...
		index = (int) (Math.random() * getMushrooms().size());
	...
	public String[] getMushroomAttributes() {
		if (distanceC > SENSING_DISTANCE)
			return null;
		return getMushrooms().get(index).getAttributes();
	...
	public void step() {
		if (action == Action.NO_ACTION) {
			...
			if (action == Action.PICK_UP) {
				if (getMushrooms().get(index).getClassification().equals("poisonous")) {
					points -= 10;
				} else {
					points += 1;
				}
	...
	public LinkedList<Mushroom> getMushrooms() {
		return mushrooms;
	}
	...
	private class Arena extends JComponent {
		...
		public void paint(Graphics g) { 
			...
			g.drawImage(getMushrooms().get(index).getImage().getImage(), mushroom.x - OBST_RADIUS / 2,
